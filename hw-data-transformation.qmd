...
composition_tidy |> filter(!is.na(food_organic_waste))
...
```{r}
composition_tidy |> filter(income_id == "HIC" | income_id == "LIC", other_percent > 40)
```
...
```{r}
composition_tidy |> 
  filter(country_name == "Switzerland" | country_name == "Ghana" | country_name == "Kenya" | country_name == "Turkey" | country_name == "Nepal" | country_name == "United States")
```
...
composition_tidy_capita <- composition_tidy |> 
  mutate(msw_capita_kg_year = msw_tons_year * 1000 / population) |> 
...
The first function mutate, allows us to create a new varible called income_cat which would have the income category levels. And the second function, case_when() allows us to assign the name of the variable to the one that was created previously in levels_income_cat, so that would assign the HIC to high income and etc.

3.  How is the object `levels_income_cat` used and what does it achieve?

    At the beginning it is used as a combination of 4 levels, and after the use on the fuction when it becomes a new variable and has its own column. And its function is just to clarify the Initials for all the income levels
...
3.  Describe what the resulting table that you can see in the Console shows you.
    ...
    It shows me the amount of times that each groups appears in the data column called income_id. For the HIC there is a freq of 81, LIC 33, LMC 47 and UMC 56

```{r}

composition_tidy_capita_income_cat |> 
  group_by(income_id) |> # los divide en grupos
  summarise(count = n()) # cuenta la cantidad de apariciones de cada division de grupo
...
2.  Observe the output in the Console and describe the difference to the resulting table of the previous code chunk.
    ...
    It is the same function as before, its counting down how many times each variable of High income, upper middle income, lower middle income and low income appears in the column income_cat. Might be a bug but it's also showing all the counters for each other column and statistics of it like minimum, quarters, maximums and NA.
...
3.  Why do you see `NA` for mean_msw_total at "high income"?
    ...
    Probably because is also counting down the NA entries in the column.
...
5.  Execute the code and observe the output in the Console. What has changed? How is that different to adding `na.rm = TRUE` as an argument to the `mean()` ?
    ...
    now it actually calculates the mean for the high income, and the primary difference is that na.rm = true just removes the na for the calculus. and the drop_na removes the NA from the columns.
...
-   `gt()`: prints the table into a viewr format
-   `tab_header()`: Allows to modify the header of the image, writing titles and subtitles
-   `fmt_number()`: Allows to specify the formats of the numbers, in this case just letting all the columns that contains mean having just one decimal
-   `starts_with()`: allows to classify the variables / columns that starts with a specific typing
-   `cols_labels()`: Allows to modify the label of the columns, instead of having silly names, we can put more direct and understandable names
...
-   `rename()`: allows us to rename the variable/columns to avoid the empty spaces of the previous titles
-   `knitr::kable()`: Makes the same table as before but printed on txt format. also letting us decide to have just 1 decimal.
